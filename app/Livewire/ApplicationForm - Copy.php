...
class ApplicationForm extends Component
{
    ...
    public function submit()
    {
        ...
        try {
            ...
            // Validate required uploads - ONLY paymentProof is required
            // All other uploads are optional (if user uploads them, they must be valid format)
            $rules['uploadedFiles.paymentProof'] = 'required|file|mimes:pdf|max:512';
            $messages['uploadedFiles.paymentProof.required'] = 'Payment Proof is required.';
            $messages['uploadedFiles.paymentProof.file'] = 'Payment Proof must be a valid file.';
            $messages['uploadedFiles.paymentProof.mimes'] = 'Payment Proof must be PDF format.';
            $messages['uploadedFiles.paymentProof.max'] = 'Payment Proof must not exceed 500KB.';

            // For optional uploads - if they are uploaded, validate format but don't require them
            foreach ($this->requiredUploads as $uploadId) {
                if ($uploadId !== 'paymentProof') {
                    // Optional uploads - validate format if present, but not required
                    if ($uploadId === 'photo') {
                        $rules['uploadedFiles.' . $uploadId] = 'nullable|image|max:200';
                    } else {
                        $rules['uploadedFiles.' . $uploadId] = 'nullable|file|mimes:pdf,jpg,jpeg,png|max:512';
                    }
                }
            }
