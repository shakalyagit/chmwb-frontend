    ...
    public function getUploadDefinitionsProperty()
    {
        return [
            'photo' => [
                ...
                'fileType' => 'JPEG',
                'maxSize' => 'Max 200KB',
                'validation' => 'required|image|max:200',
            ],
            'dobProof' => [
    ...
    public function submit()
    {
        ...
        try {
            ...
            if ($visibleFields['regNumber']) {
                ...
                $messages = ['regNumber.required' => 'Registration number is required.'];
            }
            ...
            // Validate required uploads - Use validation rules from definitions (line 156 area)
            // This ensures consistency - what's defined in getUploadDefinitionsProperty is what gets validated
            foreach ($this->requiredUploads as $uploadId) {
                $uploadDef = $this->uploadDefinitions[$uploadId];
                $validationRule = $uploadDef['validation'];

                // Use the validation rule from the definition
                $rules['uploadedFiles.' . $uploadId] = $validationRule;

                // Add error messages for required uploads
                if (strpos($validationRule, 'required') !== false) {
                    $messages['uploadedFiles.' . $uploadId . '.required'] = $uploadDef['label'] . ' is required.';
                    $messages['uploadedFiles.' . $uploadId . '.file'] = $uploadDef['label'] . ' must be a valid file.';
                    $messages['uploadedFiles.' . $uploadId . '.image'] = $uploadDef['label'] . ' must be an image file.';
                    $messages['uploadedFiles.' . $uploadId . '.mimes'] = $uploadDef['label'] . ' must be in valid format.';
                    $messages['uploadedFiles.' . $uploadId . '.max'] = $uploadDef['label'] . ' must not exceed maximum size.';
                }
            }
            ...
            $check_existing_submitted_application = ApplicationDetail::where('aadhaar', $this->aadhaar)
                ->join('application_heads', 'application_heads.id', '=', 'application_details.application_head_id')
                ->where('application_heads.status', 'submitted')
                ->exists();

            if ($check_existing_submitted_application) {
                return redirect()->back()->with(['error' => 'An application with this Aadhaar number has already been submitted.']);
            }
