...
<head>
    <meta charset="UTF-8">
    <title>Pharmacist Registration Details</title>
    <style>
        /* PDF Page Setup */
        @page {
            size: A4;
...
<body>
    ...
    @php
        ...
        $qualification = $registration && !empty($registration->qualification_name)
            ...
            : 'N/A';
        $presentAddressText = $pharmacist->presentAddress ? trim(implode(', ', array_filter([
            $pharmacist->presentAddress->address_line,
            $pharmacist->presentAddress->police_station,
            $pharmacist->presentAddress->district,
            $pharmacist->presentAddress->state,
            $pharmacist->presentAddress->pincode,
        ]))) : '';
        $presentAddressText = $presentAddressText ?: 'N/A';
        $permanentAddressText = $pharmacist->permanentAddress ? trim(implode(', ', array_filter([
            $pharmacist->permanentAddress->address_line,
            $pharmacist->permanentAddress->police_station,
            $pharmacist->permanentAddress->district,
            $pharmacist->permanentAddress->state,
            $pharmacist->permanentAddress->pincode,
        ]))) : '';
    ...
    <div class="cert-container @if(!$loop->last) page-break @endif">
        ...
        <div class="content">
            <div class="header-border">
                <h1 class="h1-title">PHARMACIST REGISTRATION DETAILS</h1>
                <p class="subtitle">
            ...
            <table class="grid-table">
                ...
                <tr>
                    <td colspan="2">
                        <div class="label">Name of the Pharmacist</div>
                        <div class="value value-name">{{ $pharmacist->name ?? 'N/A' }}</div>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <div class="label">Father / Husband Name</div>
                        <div class="value" style="font-weight: normal;">{{ $fatherName }}</div>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
